...
tp @e[name=editor,type=armor_stand] @r[name=node,type=armor_stand,score_floor_min=0,score_floor=0,score_directionCount=2,tag=!keepRoom]
scoreboard players operation @e[name=node,type=armor_stand] floor += @e[name=editor,type=armor_stand] currentFloor

#copy direction counts to temp
scoreboard objectives add temp dummy
execute @e[name=node] ~ ~ ~ scoreboard players operation @s temp = @s directionCount
...
execute @e[name=node] ~ ~ ~ scoreboard players operation @s temp -= @s directionCount
scoreboard players tag @e[name=node,type=armor_stand,score_temp_min=0] remove buildMultiRoom


#construct
function dungeon_genorator:rooms/nodes/tag_handeling/add_variation_tags if @r[name=node,type=armor_stand,score_temp=-1]
function dungeon_genorator:rooms/construction/construct if @r[name=node,type=armor_stand,score_temp=-1]
function dungeon_genorator:rooms/construction/multidirectional/construct if @r[name=node,type=armor_stand,score_temp=-1]
scoreboard players tag @e[name=node,type=armor_stand] remove buildMultiRoom if @r[name=node,type=armor_stand,score_temp=-1]
