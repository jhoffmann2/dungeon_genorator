...
execute @e[tag=extraRoom,type=armor_stand] ~ ~ ~ setblock ~ ~ ~ structure_block 0 replace {name:"clear",rotation:"NONE",posX:-4,posY:-4,posZ:-4,mirror:"NONE",ignoreEntities:0b,mode:"LOAD",showboundingbox:0b,sizeX:9,sizeY:9,sizeZ:9}
execute @e[tag=extraRoom,type=armor_stand] ~ ~ ~ setblock ~ ~1 ~ redstone_block

execute @e[name=node,type=armor_stand,score_ID=1,tag=trunk,c=1] ~ ~ ~ fill ~-55 ~-4 ~-55 ~55 ~-4 ~55 grass 0
